...
5日間でデザイン、プロタイピング、テストまでを行う手法で、スクラム開発の1つです。
これによって、終わりのないディベートや何ヶ月もかかる作業を1週間に収めることができる。

1〜4週間という短く固定されたスパン(スプリント)で動くものを作り続けることでプロダクトを開発していく
...
^投資家や、VCなども外部の専門家としてカウントされ、このような人びとは定期的な意見交流を求めてくる
...
テスト駆動型開発とは、プログラム開発手法の1つでプログラムに必要な各機能について、最初にテストを書き、そのテストが動作する必要最低限の実装をとりあえず行ったあとにコードを洗練させていくという手法。テストファーストなど、言い方は様々。
...
テストを書くためには、なるべく分割した疎結合なコードを書かなくてはいけない。
実際にテストを書いてみるとわかるが、テストを書くコストは思ってよりも高い。
しかしテストファーストであれば、テストが書きやすいような疎結合で綺麗に分割されたコードを書くことができる。
つまり結果的に保守性の高いコードを書いていくことができる。
...
テストを書くコストが高く、チームのスピード感を損ねる可能性がある。
仕様の変更が多く発生するような場合には、特にテストが追いつかない可能性が高い。
...
### GitHub
[MentorsSchool/Specification](https://github.com/MentorsSchool/Specification)
